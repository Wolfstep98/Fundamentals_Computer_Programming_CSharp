...
namespace Chapter_15_Text_Files
{
    ...
    /// <summary>
    /// Write the previous program so that it changes only the whole words (not parts of the word). c.f Exo7
    /// </summary>
    ...
    {
        static readonly string Token = " test";
        ...
        public static string RemovesTokensFromString(string buffer)
        {
            ...
            {
                index = buffer.IndexOf(Token, index, buffer.Length - index, StringComparison.CurrentCulture);
                if(index != -1)
                {
                    ...
                    buffer.CopyTo(startIndex, temp, 0, index - startIndex);
                ...
                else
                {
                    if(startIndex != 0)
                    {
                        temp = new char[buffer.Length - startIndex];
                        buffer.CopyTo(startIndex, temp, 0, buffer.Length - startIndex);
                        tempBuffer.Append(temp);
                    }
                    break;
                }
